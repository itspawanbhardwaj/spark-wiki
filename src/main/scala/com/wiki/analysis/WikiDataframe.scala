...
object WikiDataframe {
  ...
  def getDataframe(sc: SparkContext, sqlContext: SQLContext): DataFrame = {
    ...
    val separator = "\t"
    // Create an RDD
    val wiki_data = sc.textFile("sample-data/pageviews_by_second.tsv")
  ...
  def main(args: Array[String]) {
    ...
    sqlContext.sql("select * from pageviews_by_second").show(10)

    println("RUNNING SUB QUERIES ")
    sqlContext.sql("select * from (select site from pageviews_by_second) as a").show(10)
    println("finish")
  }
}